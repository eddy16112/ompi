                                    ...
                                    int max_outstanding_reqs )
{
    ...
    if( tree->tree_nextsize > 0 ) {
        ...
        for( segindex = 0; segindex <= num_segments; segindex++ ) {
            ...
            for( i = 0; i < tree->tree_nextsize; i++ ) {
             ...
             /*       ompi_datatype_type_size(datatype, &ddt_size);
                    ...
                    opal_cuda_memcpy_sync(tmp1, local_op_buffer, recvcount * ddt_size);
                    opal_cuda_memcpy_sync(tmp2, accumbuf + (ptrdiff_t)segindex * (ptrdiff_t)segment_increment, recvcount * ddt_size);
                    ompi_op_reduce(op, tmp1, tmp2, recvcount, datatype );
                    opal_cuda_memcpy_sync(local_op_buffer, tmp1, recvcount * ddt_size);
                    opal_cuda_memcpy_sync(accumbuf + (ptrdiff_t)segindex * (ptrdiff_t)segment_increment, tmp2, recvcount * ddt_size);
                    free(tmp1);
                    free(tmp2);
            ...
            /*        ompi_datatype_type_size(datatype, &ddt_size);
                    ...
                    opal_cuda_memcpy_sync(tmp3, local_op_buffer, prevcount * ddt_size);
                    opal_cuda_memcpy_sync(tmp4, accumulator, prevcount * ddt_size);
                    ompi_op_reduce(op, tmp3, tmp4, prevcount, datatype );
                    opal_cuda_memcpy_sync(local_op_buffer, tmp3, prevcount * ddt_size);
                    opal_cuda_memcpy_sync(accumulator, tmp4, prevcount * ddt_size);
                    free(tmp3);
                    free(tmp4);
        ...
        /* clean up */
        if( inbuf_free[0] != NULL) opal_cuda_free_gpu_buffer(inbuf_free[0], 0);
        if( inbuf_free[1] != NULL) opal_cuda_free_gpu_buffer(inbuf_free[1], 0);
        if( accumbuf_free != NULL ) opal_cuda_free_gpu_buffer(accumbuf_free, 0);
    }
 ...
 error_hndl:  /* error handler */
    ...
    if( inbuf_free[0] != NULL ) opal_cuda_free_gpu_buffer(inbuf_free[0], 0);
    if( inbuf_free[1] != NULL ) opal_cuda_free_gpu_buffer(inbuf_free[1], 0);
    if( accumbuf_free != NULL ) opal_cuda_free_gpu_buffer(accumbuf, 0);
    if( NULL != sreq ) {
